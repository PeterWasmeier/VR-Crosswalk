...
void fSteppermotor_Execute () {
  ...
  unsigned long ulTimeDelayBetweenTwoPulses;
  ...
  dRunningSince = dRunningSince / 1000.0;  // Value is now in seconds
  dVelocity = Steppermotor.Acceleration;   // Calculate the necessary speed of the motor
  dVelocity = dVelocity * dRunningSince;      // Convert into mm/sec (current speed to use)

  if (dVelocity>40) dVelocity=40;           // Maximum speed is 20mm/sec
  
  dPulsesPerSecond = (dVelocity * 5.0);         // Convert from mm/sec into Pulses per second
  if (dPulsesPerSecond==0.0) return;
  
  dTimeDelayBetweenTwoPulses = 1000.0 / dPulsesPerSecond; // How many milliseconds between two pulses?
  ulTimeDelayBetweenTwoPulses = dTimeDelayBetweenTwoPulses;

  if (ulDurationBetweenTwoPulses<ulTimeDelayBetweenTwoPulses) return; // The time is not yet over, so exit
  ...
  { 
    Steppermotor.ulRunningSince=0;
    if ((Steppermotor.bAutoTurnOff)&&(ulDurationBetweenTwoPulses>250)) // Turn off after 250ms
    {
