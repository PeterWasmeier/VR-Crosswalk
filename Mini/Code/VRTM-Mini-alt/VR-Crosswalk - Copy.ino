...
double dFootX=30.0;
double dFootY=0;
double dPreviousFootX;
double dPreviousFootY;
...
class tCNC_Calculator {
  ...
  public:
    ...
    long fCalculateSecondaryAxis (double dAlpha, long X1, long Y1, long X2, long Y2);
};

long tCNC_Calculator::fCalculateSecondaryAxis (double dAlpha, long X1, long Y1, long X2, long Y2) {
  double dTanAlpha;
  double dM;
  double dB;
  double dX;
  ...
  double dResult;
  long lResult;
  dTanAlpha = tan (dAlpha * ((PI/2)/180) ); // -180° ... +180° = -PI/2 .... +PI/2
  dM = (Y2-Y1) / (X2-X1);
  dB = Y1 - (dM * X1);
  dX = dB / (dTanAlpha-dM);
  dY = (dTanAlpha * dB) / (dTanAlpha - dM);
